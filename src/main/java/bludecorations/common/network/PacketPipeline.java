...
public class PacketPipeline extends MessageToMessageCodec<FMLProxyPacket, AbstractPacket>
{
	...
	public boolean registerPacket(Class<? extends AbstractPacket> clazz)
	{
		...
		{
			BluDecorations.logger.log(Level.SEVERE, "Can't register Packet: Size of PacketList is too big!");
			return false;
		}
		if (this.packets.contains(clazz))
		{
			BluDecorations.logger.log(Level.SEVERE, "Can't register Packet: Packet registered already!");
			return false;
		}
		if (this.isPostInitialised)
		{
			BluDecorations.logger.log(Level.SEVERE, "Can't register Packet: Pipeline is already post-initialized!");
			return false;
		}
	...
	{
		registerPacket(PacketRenderElement.class);
		registerPacket(PacketParticleElement.class);
		registerPacket(PacketRenderWipe.class);
		registerPacket(PacketParticleWipe.class);
		registerPacket(PacketTileRotationScale.class);
		registerPacket(PacketTileAABBLight.class);
	}
	...
	public static void writeStringToBuffer(String s, ByteBuf buffer)
	{
		char[] cTest = s.toCharArray();
		buffer.writeInt(cTest.length);
		for(int i=0;i<cTest.length;i++)
			buffer.writeInt(Character.codePointAt(cTest, i));
	}
	public static String readStringFromBuffer(ByteBuf buffer)
	{
